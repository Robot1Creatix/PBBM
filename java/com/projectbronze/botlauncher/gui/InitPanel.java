	...
	private void initMisc() {
		...
		dirSelector.addActionListener(e -> {
			if (e.getActionCommand().equals("ApproveSelection")) {
				...
				pathSelector.setFileFilter(new FileFilter() {
					...
					public boolean accept(File f) {
						return f.getAbsolutePath().contains(dir.getText()) || dir.getText().contains(f.getAbsolutePath());
					}
				});
			}
		});
		...
		create.addActionListener(e -> {
			...
			try {
				BotJson.create(dir, name.getText(), author.getText(), mainClass.getText(), Mode.valueOf(((String) this.mode.getSelectedItem()).toUpperCase()), path.getAbsolutePath().substring(dir.getAbsolutePath().length() + 1), autoRestart.isSelected());
			} catch (IOException e1) {
				error.setText("Невозможно создать бота: " + e1.getMessage());
			}
		});
	}
}
